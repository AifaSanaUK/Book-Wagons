...
const addProducts = async (req, res) => {
    try {
        const { product_name, description, category, regular_price, sale_price, quantity, offer } = req.body;
        console.log(req.body);
        console.log(req.files);

        if (!product_name || !description || !category || !regular_price || !quantity) {
...
const getAllproducts = async (req, res) => {
    try {
        ...
        console.log("Sending to EJS:", productData);
...
const addOffer = async (req, res) => {
    try {
        ...
        if (isNaN(regularPrice) || regularPrice <= 0) {
            return res.status(400).json({ status: false, message: "Invalid product price" });
        }


        const productOffer = parseFloat(percentage);
        if (isNaN(productOffer) || productOffer < 0 || productOffer > 100) {
            return res.status(400).json({ status: false, message: "Invalid product offer percentage" });
        }


        const categoryOffer = findCategory?.categoryOffer ? parseFloat(findCategory.categoryOffer) : 0;
        if (isNaN(categoryOffer)) {
            return res.status(400).json({ status: false, message: "Invalid category offer" });
        }
        ...
        if (isNaN(salePrice) || salePrice < 0) {
            return res.status(400).json({ status: false, message: "Calculated sale price is invalid" });
        }


        findProduct.salePrice = salePrice;
        findProduct.productOffer = productOffer;
        findProduct.highestOffer = highestDiscount === productDiscount ? productOffer : categoryOffer;

        await findProduct.save();

        res.json({ status: true, message: "Product offer applied successfully", salePrice });
    } catch (error) {
        console.error(error);
...
const removeOffer = async (req, res) => {
    try {
        ...
        res.json({ status: true, message: "Product offer removed successfully", salePrice });
    } catch (error) {
        console.error(error);
        res.status(500).json({ status: false, message: "Internal server error" });
    }
    ...
    try {
        ...
        ["image1", "image2", "image3"].forEach((key, index) => {
            if (req.files[key] && req.files[key].length > 0) {
                const imagePath = req.files[key][0].path.replace(/\\/g, "/").replace(/^public\//, "");
                updatedImages[index] = imagePath;
            }
        });

        console.log("Final Image Paths:", updatedImages);
...
const deleteSingleImage = async (req, res) => {
    try {
        ...
        if (!imageName || !productId) {
            return res.status(400).json({ error: "Missing image name or product ID" });
        }
        ...
        const imagePath = path.join("public", "uploads", "productImages", decodedImageName);
        console.log("Image Path:", imagePath);

        try {
            await fs.unlink(imagePath);
            console.log(`Image ${decodedImageName} deleted successfully`);
        } catch (fsError) {
            console.log(`Image ${decodedImageName} not found or failed to delete: ${fsError}`);
...
const deleteProduct = async (req, res) => {
    const { id } = req.body;
    console.log("Product ID received on the server:", id);
    if (!id) {
