...
const OtpPage = async (req, res) => {
    ...
    if (!req.session.otpData) {
        return res.render("user/forgetpassOtp", { message: { error: ["Session expired. Try signing up again."] } });
    }

    if (Date.now() > req.session.otpExpiry) {
        return res.render("user/login", { message: { error: ["OTP expired. Please request a new one."] } });
    }

    return res.render("user/forgetpassOtp", { otp: req.session.otpData.otp, message: { success: ["OTP sent successfully!"] } });
};
...
const postResetPassword = async (req, res) => {
    try {
        ...
        if (!passwordHash) {
            ...
            return res.render("user/resetpassword", { message: "Error hashing password, try again." });
        }
    ...
    try {
        ...
        if (userExists) {
            ...
            if (emailSent) {
                ...
                return res.status(200).json({ success: true, redirectUrl: "/changeemailotp" });
            } else {
