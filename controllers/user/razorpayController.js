...
const verifyPayment = async (req, res) => {
    try {
        const { razorpay_payment_id, razorpay_order_id, razorpay_signature, selectedAddressId } = req.body;
        ...
        if (!req.session.paymentData) {
            return res.status(400).json({ success: false, message: "No payment data found" });
        }
        ...
        console.log("Order saved successfully with payment ID:", razorpay_payment_id);
        ...
        try {
            for (const item of existingOrder.orderedItems) {
                ...
                await product.save();
                console.log(` Stock updated for product ${product.name} - Remaining: ${product.quantity}`);
            }
    ...
    try {
        ...
        res.render('user/paymentfailed', {
            ...
            items: order.orderedItems.map(item => ({
                ...
                image: item.product && item.product.productImage && item.product.productImage.length > 0
...
const retryPayment = async (req, res) => {
    ...
    try {
        ...
        if (!order.finalAmount) {
            return res.status(400).json({ success: false, message: "Order amount is missing." });
        }
        ...
        console.log("Retrying payment with options:", options);
        ...
        console.log("New Razorpay Order:", newOrder);
